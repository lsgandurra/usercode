...
void rangeEstimator(double percentage, TChain * chain, TString cut, double &minRange, double &maxRange, string variableName, float variable)
{
...
RooHist* residHist(RooPlot* plot, char *histname, char* curvename, bool normalize, string recordingDirectory, int iteration)
{
  ...
  if(normalize == true)
  {
        FILE *fPullsX = fopen(Form("%sPullsX_%d.txt",recordingDirectory.c_str(), iteration), "w" );
        fclose(fPullsX);
	//delete fPullsX;
        FILE *fPullsErrorX = fopen(Form("%sPullsErrorX_%d.txt",recordingDirectory.c_str(), iteration), "w" );
        fclose(fPullsErrorX);
	//delete fPullsErrorX;
        FILE *fPullsY = fopen(Form("%sPullsY_%d.txt",recordingDirectory.c_str(), iteration), "w" );
        fclose(fPullsY);
	//delete fPullsY;
        FILE *fPullsErrorY = fopen(Form("%sPullsErrorY_%d.txt",recordingDirectory.c_str(), iteration), "w" );
        fclose(fPullsErrorY);
	//delete fPullsErrorY;
  }
